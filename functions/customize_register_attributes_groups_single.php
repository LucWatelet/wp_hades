...
function customize_register_attributes_groups_single($wp_customize)
{
    $attributes = json_decode(file_get_contents(__DIR__ . '/../assets/s-attributes.json'), true);
    $attributes_groups = json_decode(file_get_contents(__DIR__ . '/../assets/attributes_groups.json'), true);
    ksort($attributes);
    ksort($attributes_groups);
    $wp_customize->add_panel(
        $id = 'hades_attributes_groups_single',
        $args = array(
            'capability'=> 'edit_theme_options',
            'description' => 'Réglages spécifiques aux attributs des offres Hadès',
            'priority' => 1,
            'theme_supports' => '',
            'title' => 'Attributs Hadès (fiche)'
        )
    );
    foreach ($attributes_groups as $attributes_group) {
        $wp_customize->add_section(
            ...
            $args = array(
                'panel' => 'hades_attributes_groups_single',
                'priority' => 1,
