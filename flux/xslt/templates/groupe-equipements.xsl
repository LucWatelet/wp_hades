...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="groupe-equipements">
    <xsl:variable name="class"      select="'hades-ul-equip'" />
    <xsl:variable name="lot"        select="'lot_equip'" />
    <xsl:variable name="ispictoenabledv"      select="$ispictoenabled" />
    <xsl:if test="count(attributs/attribut[attribute::lot=$lot])        > 0 or
                  count(equips/equip)                                   > 0 or
      ...
      <dl>
        ...
        <xsl:if test="count(equips/equip) > 0">
          <dd>
            <dl>
              <xsl:apply-templates select="equips/equip">
        ...
        <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0">
          <xsl:if test="count(equips/equip) > 0"><hr /></xsl:if>
          <dd>
            <ul class="{$class}">
        ...
        <xsl:if test="count(descriptions/description[attribute::lot=$lot]) > 0">
          <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0 or count(equips/equip) > 0"><hr /></xsl:if>
          <dd>
            <dl>
