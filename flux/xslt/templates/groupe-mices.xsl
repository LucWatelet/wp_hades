...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="groupe-mices">
    <xsl:variable name="class"      select="'hades-ul-mice'" />
    <xsl:variable name="lot"        select="'lot_mice'" />
    <xsl:variable name="ispictoenabledv"      select="$ispictoenabled" />
    <xsl:if test="count(attributs/attribut[attribute::lot=$lot])        > 0 or
                        count(mices/mice)                                     > 0 or
      ...
      <dl>
        ...
        <xsl:if test="count(descriptions/description[attribute::lot=$lot]) > 0">
          <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0 or count(mices/mice) > 0"><hr /></xsl:if>
          <dd>
            <dl>
