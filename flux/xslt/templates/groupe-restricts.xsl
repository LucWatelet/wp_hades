...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="groupe-restricts">
    <xsl:variable name="class"      select="'hades-ul-restrict'" />
    <xsl:variable name="lot"        select="'lot_restrict'" />
    <xsl:variable name="ispictoenabledv"      select="$ispictoenabled" />
    <xsl:if test="count(attributs/attribut[attribute::lot=$lot])        > 0 or
                  count(restricts/restrict)                             > 0 or
      ...
      <dl>
        ...
        <xsl:if test="count(restricts/restrict) > 0">
          <dd>
            <xsl:apply-templates select="restricts/restrict">
              <xsl:sort order="ascending" data-type="number" select="@tri" />
            </xsl:apply-templates>
          </dd>
        </xsl:if>
        <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0">
          <xsl:if test="count(restricts/restrict) > 0"><hr /></xsl:if>
          <dd>
            <ul class="{$class}">
        ...
        <xsl:if test="count(descriptions/description[attribute::lot=$lot]) > 0">
          <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0 or count(restricts/restrict) > 0"><hr /></xsl:if>
          <dd>
            <dl>
