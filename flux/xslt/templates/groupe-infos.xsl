...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="groupe-infos">
    <xsl:variable name="class"      select="'hades-ul-info'" />
    <xsl:variable name="lot"        select="'lot_info'" />
    <xsl:variable name="ispictoenabledv"      select="$ispictoenabled" />
    <xsl:if test="count(attributs/attribut[attribute::lot=$lot])        > 0 or
                  count(infos/info)                                     > 0 or
      ...
      <dl>
        ...
        <xsl:if test="count(descriptions/description[attribute::lot=$lot]) > 0">
          <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0 or count(infos/info) > 0"><hr /></xsl:if>
          <dd>
            <dl>
