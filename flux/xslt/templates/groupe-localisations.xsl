...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="groupe-localisations">
    <xsl:variable name="class"      select="'hades-ul-localisation'" />
    <xsl:variable name="lot"        select="'lot_localite'" />
    <xsl:variable name="ispictoenabledv"      select="$ispictoenabled" />
    <xsl:if test="count(attributs/attribut[attribute::lot=$lot])        > 0 or
                  count(localisation/localite)                          > 0 or
      ...
      <dl>
        ...
        <xsl:if test="count(localisation/localite) > 0">
          <dd>
            <ul class="{$class}">
              <xsl:apply-templates select="localisation/localite">
        ...
        <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0">
          <xsl:if test="count(localisation/localite) > 0"><hr /></xsl:if>
          <dd>
            <ul class="{$class}">
        ...
        <xsl:if test="count(descriptions/description[attribute::lot=$lot]) > 0">
          <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0 or count(localisations/localite) > 0"><hr /></xsl:if>
          <dd>
            <dl>
