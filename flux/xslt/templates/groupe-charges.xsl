...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="groupe-charges">
    <!-- dot, comma pour list-inline ;
         '', dash pour list-unstyled
         '' pour empty class donne les puces par défaut -->
    <xsl:variable name="class"      select="'hades-ul-charge'" />
    <xsl:variable name="lot"        select="'lot_charges'" />
    <xsl:variable name="ispictoenabledv"      select="$ispictoenabled" />
    <xsl:if test="count(attributs/attribut[attribute::lot=$lot])        > 0 or
                  count(charges/charge)                                 > 0 or
      ...
      <dl>
        ...
        <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0">
          <xsl:if test="count(charges/charge) > 0"><hr /></xsl:if>
          <dd>
            <ul class="{$class}">
        ...
        <xsl:if test="count(descriptions/description[attribute::lot=$lot]) > 0">
          <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0 or count(charges/charge) > 0"><hr /></xsl:if>
          <dd>
            <dl>
