...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="groupe-ser_grps">
    <xsl:variable name="class"      select="'hades-ul-ser-grp'" />
    <xsl:variable name="lot"        select="'lot_ser_grp'" />
    <xsl:variable name="ispictoenabledv"      select="$ispictoenabled" />
    <xsl:if test="count(attributs/attribut[attribute::lot=$lot])        > 0 or
                  count(ser_grps/ser_grp)                               > 0 or
      ...
      <dl>
        ...
        <xsl:if test="count(ser_grps/ser_grp) > 0">
          <dd>
            <xsl:apply-templates select="ser_grps/ser_grp">
              <xsl:sort order="ascending" data-type="number" select="@tri" />
            </xsl:apply-templates>
          </dd>
        </xsl:if>
        <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0">
          <xsl:if test="count(ser_grps/ser_grp) > 0"><hr /></xsl:if>
          <dd>
            <ul class="{$class}">
        ...
        <xsl:if test="count(descriptions/description[attribute::lot=$lot]) > 0">
          <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0 or count(ser_grps/ser_grp) > 0"><hr /></xsl:if>
          <dd>
            <dl>
