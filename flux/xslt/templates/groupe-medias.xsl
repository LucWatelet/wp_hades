...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="groupe-medias">
    <xsl:variable name="class"      select="'hades-ul-media'" />
    ...
    <xsl:if test="count(attributs/attribut[attribute::lot=$lot])                        > 0 or
                  count(medias/media[attribute::ext!='jpg' and attribute::ext!='png'])  > 0 or
      ...
      <dl>
        ...
        <xsl:if test="count(medias/media) > 0">
          <xsl:if test="count(medias/media[attribute::ext!='jpg' and attribute::ext!='png']) > 0">
            <dd>
              <ul class="{$class}">
                <xsl:apply-templates select="medias/media[attribute::ext!='jpg']">
        ...
        <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0">
          <xsl:if test="count(medias/media[attribute::ext!='jpg' and attribute::ext!='png']) > 0"><hr /></xsl:if>
          <dd>
            <ul class="{$class}">
        ...
        <xsl:if test="count(descriptions/description[attribute::lot=$lot]) > 0">
          <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0 or count(medias/media[attribute::ext!='jpg' and attribute::ext!='png']) > 0"><hr /></xsl:if>
          <dd>
            <dl>
