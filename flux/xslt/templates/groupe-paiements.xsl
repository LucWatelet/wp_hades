...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="groupe-paiements">
    <xsl:variable name="class"      select="'hades-ul-paiement'" />
    <xsl:variable name="lot"        select="'lot_paiement'" />
    <xsl:variable name="ispictoenabledv"      select="false()" />
    <xsl:if test="count(attributs/attribut[attribute::lot=$lot])        > 0 or
                  count(paiements/paiement)                             > 0 or
      ...
      <dl>
        ...
        <xsl:if test="count(paiements/paiement) > 0">
          <dd>
            <xsl:apply-templates select="paiements/paiement">
              <xsl:sort order="ascending" data-type="number" select="@tri" />
            </xsl:apply-templates>
          </dd>
        </xsl:if>
        <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0">
          <xsl:if test="count(paiements/paiement) > 0"><hr /></xsl:if>
          <dd>
            <ul class="{$class}">
        ...
        <xsl:if test="count(descriptions/description[attribute::lot=$lot]) > 0">
          <xsl:if test="count(attributs/attribut[attribute::lot=$lot]) > 0 or count(paiements/paiement) > 0"><hr /></xsl:if>
          <dd>
            <dl>
